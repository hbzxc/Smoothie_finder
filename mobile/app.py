...
def index():
    ...
    if request.method == 'POST':
        ...
        selected_lat = request.form.get('lat')
        selected_lon = request.form.get('lon')
        print(selected_lat)
        print(selected_lon)

        # Process the selected location and generate the map
        # ...

        return render_template('map.html', map_path='static/route_map.html')

    # Get the user's current location
    g = geocoder.ip('me')
    #user_lat, user_lon = g.latlng
    #test location
    user_lat = 52.77014219208672
    user_lon = 85.90199849548469
    ...
    if closest_location:
        ...
        folium.Marker(location=[float(lat), float(lon)], popup=f'{town_name}, {state_abbreviation}, {address}', icon=folium.Icon(color='red')).add_to(m)
        ...
        return render_template('index.html', user_lat=user_lat, user_lon=user_lon, locations=locations)
    else:
